...
bool Function SetAnimGroupCore(actor akTarget, string animGroup, int animBaseNumber, int number, string modname, bool debugOutput = false, bool skipForce3D = false, int priority = 0, int newPriority = 0) global
	if (animBaseNumber >= 0 && number >= 0 && number <= 9 && akTarget != none)
		...
		bool pass = Nemesis_AA_Reset.CompareAnimVar(akTarget, animGroup, value, priority, newPriority, akTarget.GetAnimationVariableInt("Nemesis_AA_Priority" + animGroup))

		if pass
			Debug.MessageBox(animGroup + ", Value: " + value + ", Priority: " + newPriority + ", Mod: " + modname)
		else
			Debug.MessageBox("Current running AnimGroup has higher priority than the requested AnimGroup (Mod: " + modname + ", Actor: " + (akTarget.GetBaseObject() as ActorBase) + ", Group: " + animGroup + ", Priority: " + priority + ", Old Priority: " + akTarget.GetAnimationVariableInt("Nemesis_AA_Priority" + animGroup) + ")")
			return false
		endif
		return true
	endif
