...
void nemesis::AnimationSetDataProjectTester::Run()
{
    ...
    VecNstr lines;
    lines.emplace_back("1", 1, filepath);
    lines.emplace_back("FullCharacter.txt", 2, filepath);
    lines.emplace_back("V3", 3, filepath);
    lines.emplace_back("0", 4, filepath);
    lines.emplace_back("0", 5, filepath);
    lines.emplace_back("0", 6, filepath);

    lines.emplace_back("1", 7, filepath);
    lines.emplace_back("3064642194", 8, filepath);
    lines.emplace_back("1047251415", 9, filepath);
    lines.emplace_back("7891816", 10, filepath);
    ...
    auto projects = nemesis::AnimationSetDataProject::ParseObjects(stream, manager, {"ChickenProject.txt"});
    auto m_state  = nemesis::AnimationSetDataState::Deserialize("FullCharacter.txt", m_stream, manager);

    auto asds = projects.back()->GetState("FullCharacter.txt");
    asds->MatchAndUpdate(*m_state);

    DeqNstr serialized = asds->Serialize();
    DeqNstr compiled   = projects.back()->Compile(state);
