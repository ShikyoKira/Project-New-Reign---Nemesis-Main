...
void BehaviorStart::addBehaviorPick(NemesisEngine* newWidget, vecstr behaviorOrder, unordered_map<string, bool> behaviorPick)
{
...
void BehaviorStart::GenerateBehavior(std::thread*& checkThread)
{
	...
	unordered_map<string, vector<shared_ptr<NewAnimation>>> newAnimation;						// record each of the animation created from registerAnimation
		...
		for (auto it = animationList[i]->templateType.begin(); it != animationList[i]->templateType.end(); ++it)
		{
			...
			if (BehaviorTemplate->coreTemplate[templatecode].length() > 0)
			{
				...
				else
				{
					...
					if (coreRegistered.find(corecode) == coreRegistered.end())
					{
						...
						{
							newAnimation[corecode].emplace_back(make_unique<NewAnimation>(make_shared<unordered_map<string, AnimTemplate>>(), corecode,
						...
						else
						{
							...
							newAnimation[corecode].emplace_back(make_unique<NewAnimation>(c_animTemplate, corecode, unordered_map<string, vecstr>(),
			...
			try
			{
				for (int j = 0; j < it->second; ++j)
				{
					...
					newAnimation[templatecode].emplace_back(make_unique<NewAnimation>(animTemplate, templatecode, *functionlines,
