                           ...
                           bool& negative)
{
    ...
    for (uint sect : *rows_ptr)
    {
        ...
        else
        {
            ...
            {
                processing(output[sect].back().line,
                           format,
                           filename,
                           behaviorFile,
                           nextFunctionID,
            ...
            if (pos != NOT_FOUND && line.find(")", pos) != NOT_FOUND)
            {
                ...
                int nextpos
                    = openEndBracket(templine, '(', ')', format, filename, output[sect].back().row) + 1;
    ...
    else if (hasGroup)
    {
        ...
        else
        {
            if (condition.find(format + "[") == NOT_FOUND)
            {
                ...
                {
                    if (condition.find(each + "[") == NOT_FOUND
            ...
            try
            {
                formatInfo = GetOptionInfo(condition,
                                           format,
                                           filename,
                                           storeTemplate.nested->row,
                                           groupAnimInfo,
                                           true,
                                           true,
                                           groupMulti,
        ...
        else if (condition.find(format + "_group[") != NOT_FOUND && condition.find("]") != NOT_FOUND)
        {
            ...
            if (animMulti != -1)
            {
                optionMultiLoop(output[sect],
                                ...
                                groupAnimInfo,
                                groupOrder,
                                formatInfo[2].length() == 0 ? animMulti : stoi(formatInfo[2]),
                                optionMulti,
                                ignoreGroup,
                                formatInfo[3],
                                nextFunctionID,
    ...
    for (uint i = startline; i < size; ++i)
    {
        ...
        else if (line.find("<!-- CONDITION ^", 0) != NOT_FOUND)
        {
            ...
            if (scope == 0)
            {
                ...
                storeTemplate.push_back(LineCheck(
                    make_shared<NodeJoint>(node, format, filename, behaviorFile, otherAnimType, start, i),
                    i + 1));
        ...
        else if (line.find("<!-- CLOSE -->", 0) != NOT_FOUND)
        {
            ...
            if (scope == 0)
            {
                if (startline == 0)
                {
                    ...
                    if (multiOption == format || multiOption == format + "_group"
...
void processing(string& line, shared_ptr<NodePackedParameters> parameters)
{
    processing(line,
               parameters->format,
               parameters->filename,
               parameters->behaviorFile,
               parameters->nextFunctionID,
               parameters->strID,
               parameters->groupAnimInfo,
               parameters->IDExist,
               parameters->subFunctionIDs,
               parameters->import,
               parameters->linecount,
               parameters->eventid,
               parameters->variableid,
               parameters->zeroEvent,
               parameters->zeroVariable,
               parameters->groupMulti,
               parameters->animMulti,
               parameters->optionMulti,
               parameters->hasGroup,
               parameters->multiOption,
               parameters->optionPicked,
               parameters->openRange,
               parameters->elementCount,
               parameters->line,
               parameters->elementCatch,
               parameters->negative);
}
            ...
            || line.find("#" + format + "_master") != NOT_FOUND))
    {
        ...
        for (auto& ID : generator)
        {
            if (ID.find("#" + format + "$") != NOT_FOUND && multiOption == format)
            {
                ...
                string curID
                    = std::regex_replace(string(tempID), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
                curID = format + "$" + curID;
                ...
                if (tempID.find(curID, 0) != NOT_FOUND && nextpos == line.find(curID))
                {
                    ...
                    line.replace(nextpos,
                                 curID.length(),
                                 subFunctionIDs->grouplist[groupMulti]->singlelist[animMulti]->format[curID]);
                }
                else
                {
            ...
            if (groupMulti > -1)
            {
                ...
                if (pos != string::npos)
                {
                    ...
                    if (change.find("[FilePath]") != NOT_FOUND)
                    {
                        ...
                        {
                            string number
                                = std::regex_replace(string(change.substr(change.find(format + "[", 0))),
                                                       std::regex("[^0-9]*([0-9]+).*"),
                                                       string("\\1"));
