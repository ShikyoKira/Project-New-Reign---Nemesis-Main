...
OptionList::OptionList(string filepath, string format)
{
    ...
    while (input.GetLines(strline))
    {
        ...
        if (strline.length() != 0 && strline[0] != '\'')
        {
            ...
            if (lower == "rules")
            {
                ...
                if (ruleDone) ErrorMessage(1040, format, filepath, linecount);
            ...
            else if (lower == "state")
            {
                ...
                if (stateDone) ErrorMessage(1041, format, filepath, linecount);
            ...
            else if (lower == "event")
            {
                ...
                if (templine.find("<") != NOT_FOUND && templine.find(">", templine.find("<")) != NOT_FOUND)
                {
                    ...
                    if (opening != closing) ErrorMessage(1039, format, filepath, linecount);
            ...
            else if (lower == "compulsory")
            {
                ...
                if (compulsoryDone) ErrorMessage(1045, format, filepath, linecount);
            ...
            else if (lower == "add") // Add <option> <addon> <modifier>
            {
                ...
                string key = "<" + AnimInfo[2] + ">";

                if (AnimInfo[3].find(key) == NOT_FOUND)
                {
                    ErrorMessage(1006, format, filepath, linecount, strline);
                }

                if (AnimInfo[3] == key) ErrorMessage(1005, format, filepath, linecount, strline);
            ...
            else
            {
                ...
                else
                {
                        ...
                        && strline.find(">", lineplus + 1) != NOT_FOUND)
                    {
                        ...
                        if (opening != closing) ErrorMessage(1014, format, filepath, linecount);
    ...
    for (auto it = modAddOn.begin(); it != modAddOn.end(); ++it)
    {
        ...
        if (!isAddOn[it->first])
        {
            if (storelist[it->first]) ErrorMessage(1010, format, filepath, linecount, it->first);
    ...
    {
        if (!mixOptRegis[it->first].empty()) continue;
