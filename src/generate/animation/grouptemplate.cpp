        ...
        else if (line.find("<!-- NEW ^", 0) != NOT_FOUND && line.find("^ -->", 0) != NOT_FOUND)
        {
            ...
            if (IsConditionOpened[condition])
            {
                if (!open)
                {
                    ...
                    {
                        // clear group number
                        curOption = std::regex_replace(string(optionInfo[2]),
            ...
            while (true)
            {
                ...
                if (line.find("MID$", 0) != NOT_FOUND)
                {
                    ...
                    for (int k = 0; k < counter; ++k)
                    {
                        ...
                        string ID          = std::regex_replace(string(line.substr(MIDposition)),
                                                         std::regex("[^0-9]*([0-9]+).*"),
                                                         string("\\1"));
                        string oldID       = "MID$" + ID;
                     ...
                     || line.find("#" + masterFormat + "_group$") != NOT_FOUND))
                {
                    ...
                    for (uint p = 0; p < generator.size(); p++)
                    {
                        ...
                        if (ID.find("#" + masterFormat + "$") != NOT_FOUND && multiOption == masterFormat)
                        {
                            ...
                            string curID  = std::regex_replace(
                                    ...
                                    if (!skip2 && !freeze2 && !uniqueskip2)
                                    {
                                        if (curLine.find("$") != NOT_FOUND)
                                        {
                                            ...
                                            if (curLine.find("$(S", 0) != NOT_FOUND)
                                            {
                                                ...
                                                string ID
                                                    = std::regex_replace(string(templine),
                                                                           std::regex("[^0-9]*([0-9]+).*"),
                                                                           string("\\1"));
                                                int intID;
    ...
    for (int i = 0; i < counter; ++i)
    {
        ...
        while (true)
        {
            ...
            if (change.find("[FilePath]") != NOT_FOUND)
            {
                ...
                {
                    string number
                        = std::regex_replace(string(change.substr(change.find(masterFormat + "[", 0))),
                                               std::regex("[^0-9]*([0-9]+).*"),
                                               string("\\1"));
            ...
            if (change.find("main_anim_event") != NOT_FOUND)
            {
                ...
                if (change.find(masterFormat + "[", 0) != NOT_FOUND)
                {
                    string number = std::regex_replace(
                        string(change),
                        std::regex(masterFormat + "\\[([0-9]+)\\]\\[main_anim_event\\].*"),
                        string("\\1"));
                ...
                && optionInfo.back().back() == '^' && optionInfo.back().length() > 2)
            {
                ...
                if (isalpha(optionInfo.back()[1]))
                {
                    conditionOrder = std::regex_replace(
                        string(optionInfo.back()), std::regex("\\^([A-Za-z]+)\\^"), string("\\1"));
