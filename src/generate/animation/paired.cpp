...
Paired::Paired(string line, vecstr pairedformat, int pairedcount, AOConvert import, string filepath, int stateID)
{
	bool option = false;
	...
	for (unsigned int i = 0; i < pairedlines.size(); i++)
	{
		...
		if (line.find("<!--", 0) == string::npos && !TriggerStateIgnore)
		{
			while (true)
			{
				...
				if (i > 116 && i < 138 && (!AO || AObject[1].length() == 0))
				{
					break;
				}

				if (i > 329 && i < 351 && (!AO || AObject[2].length() == 0))
				{
					break;
				}

				if (((i > 119 && i < 124) || (i > 332 && i < 337)) && HT)
				{
					break;
				}
				else if (i > 125 && i < 134)
