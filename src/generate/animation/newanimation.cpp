...
NewAnimation::NewAnimation(shared_ptr<unordered_map<string, AnimTemplate>> animlines,
                           string formatname,
                           unordered_map<string, VecStr> furnituretemplate,
                           int furniturecount,
                           string curfilepath,
...
void NewAnimation::GetNewAnimationLine(shared_ptr<VecStr> generatedlines,
                                       string curBehaviorFile,
                                       int id,
                                       ...
                                       shared_ptr<single> singleFunction,
                                       NewAnimLock& animLock)
{
    ...
    for (uint i = 0; i < fixedStateID.size(); ++i)
    {
        ...
        {
            subFunctionIDs->format["(S" + to_string(i + 1) + "+" + to_string(j) + ")"]
                = to_string(fixedStateID[i] + j);
        }
    }
    ...
    {
        if (!generatedlines->back().empty()) { generatedlines->push_back(""); }
...
void NewAnimation::processing(string& line,
                              VecStr& storeline,
                              string masterFormat,
                              int linecount,
                              ID eventid,
        ...
        while (true)
        {
                    ...
                    for (__int64 j = 0; j < maths; ++j)
                    {
                        ...
                        if (equation.find("(S", 0) != NOT_FOUND)
                        {
                            ID = std::regex_replace(
                                string(equation), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
            ...
            if (change.find("END", 0) != NOT_FOUND)
            {
                ...
                if (change.find(format + "[][END]", 0) != NOT_FOUND)
                {
                    ...
                    {
                        change.replace(change.find(format + "[][END]"),
                                       7 + format.length(),
                                       to_string(groupAnimInfo[animMulti]->duration));
                    }
                    else
                    {
                ...
                if (change.find(format + "[F][END]", 0) != NOT_FOUND)
                {
                    ...
                    {
                        change.replace(change.find(format + "[F][END]"),
                                       8 + format.length(),
                                       to_string(groupAnimInfo[0]->duration));
                    }
                    else
                    {
                ...
                if (change.find(format + "[N][END]", 0) != NOT_FOUND)
                {
                    ...
                    if (groupAnimInfo[0]->hasDuration)
                    {
                        change.replace(change.find(format + "[N][END]"),
                                       8 + format.length(),
                                       to_string(groupAnimInfo[num]->duration));
                    }
                    else
                    {
                ...
                if (change.find(format + "[B][END]", 0) != NOT_FOUND)
                {
                    ...
                    {
                        change.replace(change.find(format + "[B][END]"),
                                       8 + format.length(),
                                       to_string(groupAnimInfo[num]->duration));
                    }
                    else
                ...
                if (change.find(format + "[L][END]", 0) != NOT_FOUND)
                {
                    ...
                    {
                        change.replace(change.find(format + "[L][END]"),
                                       8 + format.length(),
                                       to_string(groupAnimInfo[lastOrder]->duration));
                    }
                    else
                    {
                ...
                if (change.find(format + "[") != NOT_FOUND)
                {
                    ...
                    if (change.find(format + "[" + number + "][END]", 0) != NOT_FOUND && isOnlyNumber(number))
                    {
                        ...
                        {
                            change.replace(change.find(format + "[" + number + "][END]"),
                                           7 + number.length() + format.length(),
            ...
            if (change.find("(S", 0) != NOT_FOUND)
            {
                ...
                string ID       = std::regex_replace(
                    string(templine), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
                int intID;
                ...
                else
                {
                    ...
                    if (intID >= int(fixedStateID.size()) || intID >= int(stateCountMultiplier.size())
                        || intID < 0)
                    {
                        ErrorMessage(1168,
                                     format,
                                     behaviorFile,
                                     linecount,
                                     templine.substr(0, templine.find(")") + 1));
                    }
                }

                if (change.find(format + "[][(S" + ID + "+", 0) != NOT_FOUND)
                {
                    stateReplacer(change,
                                  ID,
                                  fixedStateID[intID] + ((animMulti - order) * stateCountMultiplier[intID]),
                                  linecount,
                                  "",
                                  true);
                    isChange = true;
                ...
                if (change.find(format + "[F][(S" + ID + "+", 0) != NOT_FOUND)
                {
                    stateReplacer(change,
                                  ID,
                                  fixedStateID[intID] - (order * stateCountMultiplier[intID]),
                                  linecount,
                                  "F",
                                  true);
                    isChange = true;
                ...
                if (change.find(format + "[N][(S" + ID + "+", 0) != NOT_FOUND)
                {
                    stateReplacer(change,
                                  ID,
                                  fixedStateID[intID] + (isLastOrder ? 0 : stateCountMultiplier[intID]),
                                  linecount,
                                  "N",
                                  true);
                    isChange = true;
                ...
                if (change.find(format + "[B][(S" + ID + "+", 0) != NOT_FOUND)
                {
                    stateReplacer(change,
                                  ID,
                                  fixedStateID[intID] - (order == 0 ? 0 : stateCountMultiplier[intID]),
                                  linecount,
                                  "B",
                                  true);
                    isChange = true;
                ...
                if (change.find(format + "[L][(S" + ID + "+", 0) != NOT_FOUND)
                {
                    stateReplacer(change,
                                  ID,
                                  fixedStateID[intID] + ((lastOrder - order) * stateCountMultiplier[intID]),
                                  linecount,
                                  "L",
                                  true);
                    isChange = true;
                ...
                if (change.find(format + "[") != NOT_FOUND)
                {
                    ...
                    if (change.find(format + "[" + number + "][(S" + ID + "+") != NOT_FOUND
                        && isOnlyNumber(number))
                    {
                        string number2 = std::regex_replace(
                            ...
                            != NOT_FOUND)
                        {
                            stateReplacer(change,
                                          ID,
                                          fixedStateID[intID]
                                              + ((stoi(number) - order) * stateCountMultiplier[intID]),
                                          linecount,
                                          number,
                                          true);
                            isChange = true;
            ...
            if (change.find("FilePath", 0) != NOT_FOUND)
            {
                if (change.find(format + "[][FilePath]", 0) != NOT_FOUND)
                {
                    ...
                    change.replace(change.find(format + "[][FilePath]"),
                                   12 + format.length(),
                                   filepath + groupAnimInfo[animMulti]->filename);
                    isChange = true;
                }

                if (change.find(format + "[F][FilePath]", 0) != NOT_FOUND)
                {
                    change.replace(change.find(format + "[F][FilePath]"),
                                   13 + format.length(),
                                   filepath + groupAnimInfo[0]->filename);
                    isChange = true;
                }

                if (change.find(format + "[N][FilePath]", 0) != NOT_FOUND)
                {
                    change.replace(change.find(format + "[N][FilePath]"),
                                   13 + format.length(),
                                   filepath + (isLastOrder ? filename : groupAnimInfo[order + 1]->filename));
                    isChange = true;
                }

                if (change.find(format + "[B][FilePath]", 0) != NOT_FOUND)
                {
                    change.replace(change.find(format + "[B][FilePath]"),
                                   13 + format.length(),
                                   filepath + (order == 0 ? filename : groupAnimInfo[order - 1]->filename));
                    isChange = true;
                }

                if (change.find(format + "[L][FilePath]", 0) != NOT_FOUND)
                {
                    change.replace(change.find(format + "[L][FilePath]"),
                                   13 + format.length(),
                                   filepath + groupAnimInfo[lastOrder]->filename);
                    isChange = true;
                }
                ...
                {
                    string number = std::regex_replace(
                        string(change), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));

                    if (change.find(format + "[" + number + "][FilePath]", 0) != NOT_FOUND
                        && isOnlyNumber(number))
                    {
                        change.replace(change.find(format + "[" + number + "][FilePath]"),
                                       12 + format.length() + number.length(),
                                       filepath + groupAnimInfo[stoi(number)]->filename);
                        isChange = true;
                    }
                }
            ...
            if (change.find("FileName", 0) != NOT_FOUND)
            {
                if (change.find(format + "[][FileName]", 0) != NOT_FOUND)
                {
                    ...
                    change.replace(change.find(format + "[][FileName]"),
                                   12 + format.length(),
                                   groupAnimInfo[animMulti]->filename.substr(
                                       0, groupAnimInfo[animMulti]->filename.find_last_of(".")));
                    isChange = true;
                }
                ...
                {
                    change.replace(change.find(format + "[N][FileName]"),
                                   13 + format.length(),
                                   (isLastOrder
                                        ? filename.substr(0, filename.find_last_of("."))
                                        : groupAnimInfo[order + 1]->filename.substr(
                                            0, groupAnimInfo[order + 1]->filename.find_last_of("."))));
                    isChange = true;
                }

                if (change.find(format + "[B][FileName]", 0) != NOT_FOUND)
                {
                    change.replace(change.find(format + "[B][FileName]"),
                                   13 + format.length(),
                                   (order == 0 ? filename.substr(0, filename.find_last_of("."))
                                               : groupAnimInfo[order - 1]->filename.substr(
                                                   0, groupAnimInfo[order - 1]->filename.find_last_of("."))));
                    isChange = true;
                }
                ...
                {
                    change.replace(change.find(format + "[L][FileName]"),
                                   13 + format.length(),
                                   groupAnimInfo[lastOrder]->filename.substr(
                                       0, groupAnimInfo[lastOrder]->filename.find_last_of(".")));
                    isChange = true;
                }
                ...
                if (change.find(format + "[", 0) != NOT_FOUND)
                {
                        ...
                        && isOnlyNumber(number))
                    {
                        change.replace(change.find(format + "[" + number + "][FileName]"),
                                       12 + format.length() + number.length(),
                                       groupAnimInfo[stoi(number)]->filename.substr(
                                           0, groupAnimInfo[stoi(number)]->filename.find_last_of(".")));
                        isChange = true;
                    }
                }
                ...
                if (change.find("FileName", 0) != NOT_FOUND)
                {
                    ...
                    change.replace(
                        change.find("FileName"), 8, filename.substr(0, filename.find_last_of(".")));
                    isChange = true;
                }
            ...
            if (change.find("Path", 0) != NOT_FOUND)
            {
                if (change.find(format + "[][Path]", 0) != NOT_FOUND)
                {
                    ...
                    change.replace(change.find(format + "[][Path]"),
                                   8 + format.length(),
                ...
                {
                    change.replace(change.find(format + "[F][Path]"),
                                   9 + format.length(),
                                   filepath.substr(0, filepath.length() - 1));
                    isChange = true;
                }
                ...
                {
                    change.replace(change.find(format + "[N][Path]"),
                                   9 + format.length(),
                                   filepath.substr(0, filepath.length() - 1));
                    isChange = true;
                }
                ...
                {
                    change.replace(change.find(format + "[B][Path]"),
                                   9 + format.length(),
                                   filepath.substr(0, filepath.length() - 1));
                    isChange = true;
                }
                ...
                {
                    change.replace(change.find(format + "[L][Path]"),
                                   9 + format.length(),
                                   filepath.substr(0, filepath.length() - 1));
                    isChange = true;
                }
                ...
                if (change.find(format + "[", 0) != NOT_FOUND)
                {
                    ...
                    if (change.find(format + "[" + number + "][Path]", 0) != NOT_FOUND
                        && isOnlyNumber(number))
                    {
                        change.replace(change.find(format + "[" + number + "][Path]"),
            ...
            if (change.find("@AnimObject/", 0) != NOT_FOUND)
            {
                    ...
                    && change.find("][@AnimObject/", 0) != NOT_FOUND)
                {
                    ...
                    for (int k = 0; k < counter; ++k)
                    {
                        ...
                        VecStr optionInfo = GetOptionInfo(change.substr(nextpos, pos - nextpos),
                                                          format,
                                                          behaviorFile,
                                                          linecount,
                                                          lastOrder,
                        ...
                        if (optionInfo[0] == format && optionInfo[2].find("@AnimObject/") != NOT_FOUND)
                        {
                            ...
                            animObjectReplacer(change,
                                               behaviorFile,
                                               groupAnimInfo[groupNum]->AnimObject,
                                               format,
                                               linecount,
                                               optionMulti,
                                               true,
            ...
            if (change.find("main_anim_event", 0) != NOT_FOUND)
            {
                    ...
                    && change.find("eventID[" + format + "[][main_anim_event]]") == NOT_FOUND)
                {
                    change.replace(change.find("[" + format + "[][main_anim_event]]"),
                                   21 + format.length(),
                                   groupAnimInfo[animMulti]->mainAnimEvent);
                    isChange = true;
                }
                    ...
                    && change.find("eventID[" + format + "[N][main_anim_event]]", 0) == NOT_FOUND)
                {
                    change.replace(change.find("[" + format + "[N][main_anim_event]]"),
                                   22 + format.length(),
                                   (isLastOrder ? mainAnimEvent : groupAnimInfo[order + 1]->mainAnimEvent));
                    isChange = true;
                }
                    ...
                    && change.find("eventID[" + format + "[L][main_anim_event]]", 0) == NOT_FOUND)
                {
                    change.replace(change.find("[" + format + "[L][main_anim_event]]"),
                                   22 + format.length(),
                                   groupAnimInfo[lastOrder]->mainAnimEvent);
                    isChange = true;
                }
                ...
                string test = std::regex_replace(
                    string(change),
                    std::regex(".*" + format + "\\[([0-9]+)\\]\\[main_anim_event\\].*"),
                    string("\\1"));
                ...
                if (test != change)
                {
                               ...
                               == NOT_FOUND)
                    {
                        if (isLastOrder)
                        {
                            change.replace(change.find("[" + format + "[" + test + "][main_anim_event]]"),
                                           21 + format.length() + test.length(),
                                           mainAnimEvent);
                        }
                        else
                        {
            ...
            if (change.find("LastState", 0) != NOT_FOUND)
            {
                ...
                {
                    number = std::regex_replace(string(change.substr(change.find("LastState"))),
                                                  std::regex("[^0-9]*([0-9]+).*"),
                                                  string("\\1"));
                ...
                change.replace(change.find("LastState"),
                               9 + number.length(),
                               to_string(lastOrder - order + fixedStateID[ID]));
                isChange = true;
            }
...
void addOnReplacer(string& line,
                   string filename,
...
void animObjectReplacer(string& line,
                        string filename,
                        unordered_map<int, VecStr> AnimObject,
                        string format,
                        int linecount,
                        int optionMulti,
                        bool otherAnim,
                        string animNum)
{
    if (otherAnim)
    {
        ...
        for (int k = 0; k < reference; ++k)
        {
            ...
            string number
                = std::regex_replace(string(object), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
...
void NewAnimation::stateReplacer(
    string& line, std::string statenum, int stateID, int linecount, string otherAnimOrder, bool otherAnim)
{
    if (otherAnim)
    {
        string number = std::regex_replace(
    ...
    else
    {
        ...
        string number   = std::regex_replace(
            string(templine.substr(statenum.length() + 3)), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
...
bool NewAnimation::addOnConverter(
    int& optionMulti, int& endMulti, VecStr& optionInfo, shared_ptr<AnimationInfo> animInfo, bool second)
{
    ...
    if (addOnPtr->find(optionInfo[2]) != addOnPtr->end() && (*addOnPtr)[optionInfo[2]].size() > 0)
    {
        ...
        if (groupAdditionPtr->find(optionInfo[3]) != groupAdditionPtr->end()
            && (*groupAdditionPtr)[optionInfo[3]].size() > 0)
        {
    ...
    string& line, string format, string filename, ID eventid, string firstEvent, int linecount)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        string eventName     = std::regex_replace(
            string(fullEventName), std::regex(".*eventID[[](.*?)[]].*"), string("\\1"));
    ...
    string& line, string format, string filename, ID variableid, string ZeroVariable, int linecount)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        string varName     = std::regex_replace(
            string(fullVarName), std::regex(".*variableID[[](.*)[]].*"), string("\\1"));
...
VecStr GetOptionInfo(string line,
                     ...
                     int numline,
                     size_t lastOrder,
                     vector<shared_ptr<AnimationInfo>> groupAnimInfo,
                     bool allowNoFixAnim,
                     bool isCondition,
                     int order,
                     int animMulti,
                     int optionMulti)
{
    ...
    if (line.find(format + "[") != NOT_FOUND && groupAnimInfo.size() != 0)
    {
        ...
        else
        {
            ...
            templine = templine + "a";
            string newtempline
                = std::regex_replace(string(templine), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
        ...
        if (!isCondition && optionInfo.size() > 2
            && (optionInfo[2] == "main_anim_event" || optionInfo[2] == "[main_anim_event]"
                || optionInfo[2] == "FilePath" || optionInfo[2].find("@AnimObject") != NOT_FOUND
                || optionInfo[2].find("(S+") != NOT_FOUND))
            limiter--;
...
{
    string templine
        = std::regex_replace(string(option2), std::regex("[^A-Za-z\\s]*([A-Za-z\\s]+).*"), string("\\1"));
                                    ...
                                    AnimationUtility utility)
{
    ...
    if (condition.find(format + "[][") != NOT_FOUND)
    {
        ...
        while (formatGroup < int(groupAnimInfo.size()))
        {
            if (utility.animMulti == -1)
                optionInfo = GetOptionInfo(condition,
                                           format,
                                           behaviorFile,
                                           numline,
                                           groupOptionPicked.size() - 1,
                                           groupAnimInfo,
                                           true,
                                           true,
                                           order,
    ...
    {
        optionInfo = utility.animMulti == -1 ? GetOptionInfo(condition,
                                                             format,
                                                             behaviorFile,
                                                             numline,
                                                             groupOptionPicked.size() - 1,
                                     ...
                                     bool isNot)
{
    if (optionInfo[2][0] == '^' && optionInfo[2].back() == '^')
    {
        ...
        {
            conditionOrder = std::regex_replace(
                string(optionInfo[2]), std::regex("\\^([A-Za-z]+)\\^"), string("\\1"));
            ...
            if (IsConditionOpened[condition])
            {
                if (!open)
                {
                    ...
                    VecStr optionInfo = GetOptionInfo(
                        curOption, format, behaviorFile, i + 1, lastOrder, groupAnimInfo, false, true, order);
                    ...
                    if (optionInfo[2] != "AnimObject")
                    {
                        ...
                        else
                        {
                            ...
                            string templine
                                = std::regex_replace(string(optionInfo[2]),
                            ...
                            {
                                string ID = std::regex_replace(
                                    string(previous), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
        ...
        else if (line.find("<!-- FOREACH ^", 0) != NOT_FOUND)
        {
            ...
            if (IsConditionOpened[condition])
            {
                if (!open)
                {
                    ...
                    while (true)
                    {
                        VecStr optionInfo = GetOptionInfo(curOption,
                                                          format,
                                                          behaviorFile,
                                                          i + 1,
                                                          groupAnimInfo.size() - 1,
                                                          groupAnimInfo,
                                                          true,
                                                          true,
                                                          order);
                            ...
                            for (uint k = 0; k < recorder.size(); ++k)
                            {
                                        ...
                                        if (!IsConditionOpened[condition])
                                        {
                                            if (isPassed(condition, IsConditionOpened))
                                            {
                                                size_t optionPosition
                                                    = newline.find("<!-- CONDITION START ^") + 22;
                                                string conditionLine = newline.substr(
                                                    optionPosition,
                                                    newline.find("^ -->", optionPosition) - optionPosition);
                                                AnimationUtility utility;
                                                utility.animMulti = animMulti;

                                                if (conditionLine.find("[") == NOT_FOUND
                                                    || conditionLine.find("]") == NOT_FOUND)
                                                {
                                                    ErrorMessage(
                                        ...
                                        if (!IsConditionOpened[condition])
                                        {
                                            if (isPassed(condition, IsConditionOpened))
                                            {
                                                ...
                                                size_t conditionPosition
                                                    = newline.find("<!-- CONDITION") + 14;
                                                string replacement1
                                                    = newline.substr(0, conditionPosition + 1);
                                ...
                                if (!skip2 && !uniqueskip2 && !freeze2)
                                {
                                    if (newline.find("$") != NOT_FOUND)
                                    {
                                        ...
                                        {
                                            newline.replace(
                                                newline.find("$%$"), 3, to_string(furnitureCount));
                                        }
                                        ...
                                        if (newline.find("$MC$", 0) != NOT_FOUND)
                                        {
                                            ID emptyID;
                                            vector<int> emptyVI;
                                            AnimationUtility utility(newline,
                                                                     emptyID,
                                                                     emptyID,
                                                                     emptyVI,
                                                                     ASD,
                                    ...
                                    if (pos != NOT_FOUND && newline.find(")", pos) != NOT_FOUND)
                                    {
                                        ...
                                        int nextpos
                                            = openEndBracket(templine, '(', ')', format, behaviorFile, i + 1)
                                              + 1;
...
void NewAnimation::AnimDataLineProcess(AnimTemplate* originaltemplate,
...
void motionDataReplacer(string& change,
                        string format,
                        string filename,
                        string behaviorFile,
                        int linecount,
                        ...
                        bool& isChange)
{
    ...
    {
        string number
            = std::regex_replace(string(change), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
        ...
        if (change.find(format + "[" + number + "][MD]", 0) != NOT_FOUND && isOnlyNumber(number))
        {
            ...
            change.replace(change.find(format + "[" + number + "][MD]"),
                           6 + format.length() + number.length(),
                           motionData);
            isChange = true;
        }
    }
...
void rotationDataReplacer(string& change,
                          string format,
                          string filename,
                          string behaviorFile,
                          int linecount,
                          ...
                          bool& isChange)
{
    ...
    if (change.find(format + "[", 0) != NOT_FOUND)
    {
        ...
        if (change.find(format + "[" + number + "][RD]", 0) != NOT_FOUND && isOnlyNumber(number))
        {
            ...
            change.replace(change.find(format + "[" + number + "][RD]"),
                           6 + format.length() + number.length(),
                           rotationData);
            isChange = true;
        }
...
bool isEquation(string& line)
{
    ...
    {
        if (!isdigit(curChar) && curChar != '*' && curChar != '+' && curChar != '-' && curChar != '/'
            ...
            while (condcount < curstack.nestedcond.size())
            {
                ...
                else if (curcond.isOrder)
                {
                    ...
                    else
                    {
                        ...
                        if (word & number) { ErrorMessage(1110, format, behaviorFile, curcond.linenum); }
                        else if (unknown)
                        {
...
void NewAnimation::hasProcessing(string& line,
                                 bool& norElement,
                                 int& openRange,
                                 int numline,
                                 shared_ptr<VecStr> generatedlines,
                                 ...
                                 int animMulti)
{
    if (!hasDuration && isEnd)
    {
        ...
        {
            line.replace(
                line.find("\t\t<hkparam name=\"relativeToEndOfClip\">false</hkparam>") + 38, 5, "true");
            isEnd = false;
        }
    ...
    else if (line.find("</hkparam>") != NOT_FOUND && norElement)
    {
        ...
        if (openRange == range)
        {
            ...
            if ((*generatedlines)[elementLine].find("numelements=\"$elements$\">", 0) == NOT_FOUND)
            {
                ...
                oldElement      = (*generatedlines)[elementLine].substr(
                    position, (*generatedlines)[elementLine].find("\">", position) - position);
            }
            else
            {
                oldElement = "$elements$";
            ...
            {
                (*generatedlines)[elementLine].replace(
                    (*generatedlines)[elementLine].find(oldElement), oldElement.length(), to_string(counter));
            }
    ...
    if (line.find("$") != NOT_FOUND)
    {
        ...
        if (line.find(format + "_group$") != NOT_FOUND)
        {
            ...
            for (int k = 0; k < counter; ++k)
            {
                ...
                string ID = std::regex_replace(
                    string(line.substr(pos)), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
                string oldID  = "MID$" + ID;
                ...
                if (newpos == pos)
                {
                    ...
                    {
                        line.replace(
                            pos, format.length() + 7 + ID.length(), groupFunction->functionIDs[oldID]);
                    }
                    else
                    {
        ...
        if (line.find("MID$", 0) != NOT_FOUND)
        {
            ...
            for (int k = 0; k < counter; ++k)
            {
                ...
                string ID          = std::regex_replace(
                    string(line.substr(MIDposition)), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
                string oldID = "MID$" + ID;
