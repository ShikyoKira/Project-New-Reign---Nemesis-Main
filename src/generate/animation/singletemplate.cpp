                           ...
                           nemesis::CondVar<string>* generatedlines)
{
    ...
    if (line.find("$") != NOT_FOUND)
    {
        ...
        if (pos != NOT_FOUND)
        {
            vector<nemesis::MultiChoice_Old> m_conditions;
            process.hasMC.insert(numline);

            for (nemesis::regex_iterator itr(line, nemesis::regex("[\\s]+<!-- (.+?) -->[\\s]*?"));
                 ...
                 ++itr)
            {
                ...
                {
                    m_conditions.emplace_back(nemesis::MultiChoice_Old("",
                                                                format,
                                                                behaviorFile,
                                                                multiOption,
                                                                numline,
                ...
                else if (curElements.size() > 1)
                {
                    ...
                    m_conditions.emplace_back(nemesis::MultiChoice_Old(curElements[0],
                                                                format,
                                                                behaviorFile,
                                                                multiOption,
                                                                numline,
            ...
            for (nemesis::regex_iterator itr(line, nemesis::regex("\\$MC\\$"));
                 itr != nemesis::regex_iterator();
                 ++itr)
            {
                pos = itr->position();
        ...
        if (isGroup)
        {
            for (nemesis::regex_iterator itr(line, nemesis::regex(format + "\\$([0-9]+)"));
                 itr != nemesis::regex_iterator();
                 ++itr)
            {
                string ID = itr->str(1);
            ...
            for (nemesis::regex_iterator itr(line, nemesis::regex(format + "_group\\$([0-9]+)"));
                 itr != nemesis::regex_iterator();
                 ++itr)
            {
                string ID = itr->str(1);
        ...
        else
        {
                 ...
                 ++itr)
            {
                ...
                pos       = itr->position();
                process.installBlock(
                    nemesis::scope(pos, pos + itr->str().length(), VecStr{ID}, &proc::groupIDRegis), numline);
                hasProcess = true;
            }
        }
        ...
        if (pos != NOT_FOUND)
        {
            ...
            for (nemesis::regex_iterator itr(line, nemesis::regex("MID\\$([0-9]+)"));
                 itr != nemesis::regex_iterator();
                 ++itr)
            {
                pos = itr->position();
                process.installBlock(
                    nemesis::scope(pos, pos + itr->str().length(), VecStr{itr->str(1), line}, func), numline);
                hasProcess = true;
            }
        }
    }
    ...
    if (isEnd)
    {
        for (nemesis::regex_iterator itr(
                  ...
                  void (proc::*func)(nemesis::scope, VecStr&, AnimThreadInfo&) const)
{
    ...
    for (nemesis::regex_iterator itr(change, expr); itr != nemesis::regex_iterator(); ++itr)
    {
        string ID     = itr->str(1);
                          ...
                          proc& process)
{
    for (nemesis::regex_iterator itr(change, expr); itr != nemesis::regex_iterator(); ++itr)
    {
        ...
        {
            isGroup ? process.installBlock(
                nemesis::scope(post, post + itr->str().length(), VecStr{itr->str()}, func), numline)
                     ...
                     proc& process)
{
    if (isMaster && multiOption != format + "_group")
    {
        ...
        for (nemesis::regex_iterator itr(change, nemesis::regex(format + "_group\\[(.*?)\\]"));
             itr != nemesis::regex_iterator();
             ++itr)
        {
    ...
    if (change.find("(S", 0) != NOT_FOUND)
    {
        ...
        for (nemesis::regex_iterator itr(change, nemesis::regex(expstr));
             itr != nemesis::regex_iterator();
             ++itr)
        {
            string first = itr->str(1);
    ...
    if (change.find("main_anim_event", 0) != NOT_FOUND)
    {
        mainAnimEventInstall(
            ...
            numline,
            curPos,
            nemesis::regex("\\{" + shortcut + "\\[(F|N|B|L|[0-9]*)\\]\\[main_anim_event\\]\\}"),
            isGroup,
            isMaster,
            process);
        mainAnimEventInstall(
        ...
        for (nemesis::regex_iterator itr(change, nemesis::regex("(\\{main_anim_event\\})"));
             itr != nemesis::regex_iterator();
             ++itr)
        {
            if (isGroup && multiOption != format) ErrorMessage(1146, format, behaviorFile, numline);
    ...
    if (pos != NOT_FOUND && change.find("]", pos) != NOT_FOUND)
    {
        for (auto it = optionlist.addOn.begin(); it != optionlist.addOn.end(); ++it)
        {
            for (auto& addname : it->second)
            {
                ...
                for (nemesis::regex_iterator itr(change,
    ...
    {
        for (nemesis::regex_iterator itr(change, nemesis::regex("LastState([0-9]*)"));
             itr != nemesis::regex_iterator();
             ++itr)
        {
    ...
    if (change.find("HkxEvent[") != NOT_FOUND)
    {
        ...
        while (pos != NOT_FOUND)
        {
            ...
            for (size_t ch = eventpos; ch < change.length(); ++ch)
            {
                if (change[ch] == '[')
                    ++curcounter;
                else if (change[ch] == ']')
    ...
    if (change.find("variableID[") != NOT_FOUND)
    {
        ...
        while (pos != NOT_FOUND)
        {
            ...
            for (size_t ch = varpos; ch < change.length(); ++ch)
            {
                if (change[ch] == '[')
                    ++curcounter;
                else if (change[ch] == ']')
    ...
    if (change.find("animOrder[") != NOT_FOUND)
    {
        ...
        while (pos != NOT_FOUND)
        {
            ...
            for (size_t ch = animpos; ch < change.length(); ++ch)
            {
                if (change[ch] == '[')
                    ++curcounter;
                else if (change[ch] == ']')
    ...
    if (change.find("crc32[") != NOT_FOUND)
    {
        ...
        while (pos != NOT_FOUND)
        {
            ...
            for (size_t ch = encode; ch < change.length(); ++ch)
            {
                if (change[ch] == '[')
                    ++curcounter;
                else if (change[ch] == ']')
    ...
    if (change.find("import[") != NOT_FOUND)
    {
        ...
        while (pos != NOT_FOUND)
        {
            ...
            for (size_t ch = pos; ch < change.length(); ++ch)
            {
                if (change[ch] == '[')
                    ++curcounter;
                else if (change[ch] == ']')
    ...
    {
        for (nemesis::regex_iterator itr(change, nemesis::regex(format + "\\[(F|N|B|L|[0-9]*)\\]\\[MD\\]"));
             itr != nemesis::regex_iterator();
             ++itr)
        {
            bool number  = false;
        ...
        for (nemesis::regex_iterator itr(change,
                                         nemesis::regex("(?<!" + format + "\\[[F|N|B|L|\\d]\\]\\[)(?<!"
                                                        + format + "\\[\\]\\[)(?<!" + format
                                                        + "\\[\\d\\d\\]\\[)(MD)"));
             itr != nemesis::regex_iterator();
             ++itr)
    ...
    {
        for (nemesis::regex_iterator itr(change, nemesis::regex(format + "\\[(F|N|B|L|[0-9]*)\\]\\[RD\\]"));
             itr != nemesis::regex_iterator();
             ++itr)
        {
            bool number  = false;
