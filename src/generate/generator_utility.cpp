        ...
        for (auto it = mainJoint.begin(); it != mainJoint.end(); ++it)
        {
            if (functionState.find(it->second) == functionState.end())
            {
                ...
                for (uint j = 0; j < functionlist[it->second].size(); ++j)
                {
                    ...
                    if (!rightFunction) { ErrorMessage(1077); }
                    else if (open)
                    {
        ...
        if (std::filesystem::is_directory(animationDirectory + file1))
        {
            ...
            if (isFileExist(animationDirectory + file1 + "\\" + targetfile))
            {
                list.emplace_back(make_unique<registerAnimation>(
                    targetdir, targetfile, behaviortemplate, modBehavior, behaviorfile, firstP));
            }
            ...
            {
                list.emplace_back(make_unique<registerAnimation>(
                    targetdir, targetfile, behaviortemplate, modBehavior, behaviorfile, firstP, true));
            }
        }
...
vector<unique_ptr<registerAnimation>> openFile(TemplateInfo* behaviortemplate)
{
    ...
    for (auto& behaviorGroup : behaviortemplate->grouplist)
    {
        ...
        if (pos != NOT_FOUND && nextpos != NOT_FOUND && nextpos < pos)
            animPath.insert(string(path.substr(nextpos, pos - nextpos)) + '\\');
        else if (behaviorGroup.first != "animationdatasinglefile"
                 && behaviorGroup.first != "animationsetdatasinglefile")
            WarningMessage(1007, behaviorGroup.first, path);
    ...
    try
    {
        for (auto& file : filelist)
        {
            ...
            else if (nemesis::iequals(curfile.extension().string(), ".txt"))
            {
                ...
                {
                    if (std::filesystem::path(directory).stem().string().find("~") != NOT_FOUND
...
bool isEngineUpdated(string& versionCode)
{
    ...
    for (auto& line : storeline)
    {
        if (line.length() > 0)
        {
            ...
            if (!isFileExist(part1))
                return false;
            else if (GetLastModified(part1) != part2)
                return false;
...
bool isEdited(TemplateInfo* BehaviorTemplate,
              ...
              unordered_map<string, vector<shared_ptr<NewAnimation>>>& newAnimation,
              bool isCharacter,
              string modID)
{
    ...
    if (isCharacter)
    {
        if ((lowerBehaviorFile == "defaultfemale" || lowerBehaviorFile == "defaultmale")
            && animReplaced.size() > 0)
            return true;
...
void checkAllStoredHKX()
{
    ...
    {
        if (isFileExist(file)) { fileArchitectureCheck(file); }
    }
...
void checkFolder(string filepath)
{
    ...
    for (auto& each : list)
    {
        ...
        if (std::filesystem::is_directory(file)) { checkFolder(file.string()); }
