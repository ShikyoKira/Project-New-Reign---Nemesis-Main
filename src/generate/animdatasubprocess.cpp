                ...
                if (l + 3 < catalyst.size() && l > 2)
                {
                        ...
                        if (!out)
                        {
                            if (!isInfo)
                            {
                                if (hasAlpha(line))
                                {
                                    ...
                                    {
                                        string number
                                            = std::regex_replace(string(catalyst[l + 1]),
                                ...
                                else if (isOnlyNumber(line)) // is info
                                {
                                    ...
                                    newline.reserve(20);
                                    newline.clear();
                                    string number = std::regex_replace(string(catalyst[++l]),
                                                                         std::regex("[a-zA-Z]+[$]([0-9]+)"),
                                                                         string("\\1"));
                            ...
                            else // new info added by mod
                            {
                                string number = std::regex_replace(
                                    string(line), std::regex("[a-zA-Z]+[$]([0-9]+)"), string("\\1"));
                    ...
                    else if (header == "$header$")
                    {
                        if (hasAlpha(line) && line.find("\\") == NOT_FOUND && l + 1 < catalyst.size())
                        {
                            ...
                            {
                                string number = std::regex_replace(string(catalyst[l + 1]),
                                                                     std::regex("[a-zA-Z]+[$]([0-9]+)"),
                                                                     string("\\1"));
