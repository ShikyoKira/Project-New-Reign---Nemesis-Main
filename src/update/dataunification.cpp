...
bool newAnimUpdateExt(string folderpath,
                      string modcode,
                      string behaviorfile,
                      map<string, VecStr, alphanum_less>& newFile,
                      map<string, VecStr>& newAnimAddition,
        ...
        for (auto& curline : storeline)
        {
            ...
            if (curline.find("SERIALIZE_IGNORED") == NOT_FOUND)
            {
                ...
                if (curline.find("			#") != NOT_FOUND
                    && newline.back().find("numelements=\"", 0) != NOT_FOUND)
                    start = true;
                        ...
                        || originallines[linecount].find("<!-- FOREACH", 0) != NOT_FOUND)
                    {
                        ...
                        while (oriScope != 0)
                        {
                            ...
                            if (linecount == originallines.size() && oriScope != 0)
                                ErrorMessage(1114, modcode, curfile.string(), i + 1);
                        }
                    }
                    else if (curline.find("<!-- ", 0) != NOT_FOUND
                             && curline.find("numelements +", curline.find("<!-- ")) != NOT_FOUND)
                    {
                        size_t position = curline.find("<!-- ");
                        string templine
                            = curline.substr(position, curline.find("-->", position) - position + 3);

                        if (originallines[linecount].find("<!-- ", 0) != NOT_FOUND
                            && originallines[linecount].find("-->", 0) != NOT_FOUND)
                          ...
                          unordered_map<string, string>& lastUpdate)
{
    ...
    CombineAnimData(folderpath,
                    "$haeder$",
                    modcode,
                    GetFileDirectory(folderpath) + "\\$header$",
                    storeline,
                    animData.animDataChar,
                    animData,
                    true);
                          ...
                          unordered_map<string, string>& lastUpdate)
{
    ...
    for (uint k = 0; k < headerlist.size(); ++k)
    {
        ...
        bool isinfo = false;
        string tempname
            = std::regex_replace(string(filename), std::regex("[^~]*~([0-9]+)"), string("\\1"));
        ...
        if (characterfile.find("~") != NOT_FOUND)
        {
            ...
            string tempproject
                = std::regex_replace(string(characterfile), std::regex("~([0-9]+)"), string("\\1"));
        ...
        {
            if (!isinfo)
                animData.animDataHeader[project].push_back(filename); // anim data
            else
                animData.animDataInfo[project].push_back(filename); // info data
        ...
        if (!isNew)
        {
            CombineAnimData(filename,
                            characterfile,
                            modcode,
                            filepath,
                            storeline,
                            animData.newAnimData[project][filename],
                            animData,
                            false);
        }
        else if (filename[0] == '$'
                             ...
                             unordered_map<string, string>& lastUpdate)
{
    ...
    for (uint k = 0; k < headerfile.size(); ++k)
    {
        ...
        if (std::filesystem::is_directory(curfile) || !nemesis::iequals(curfile.extension().string(), ".txt")
            || headerfile[k].length() == 0)
            continue;
            ...
            != animSetData.newAnimSetData[projectfile].end())
        {
            ...
            for (string& line : storeline)
            {
                ...
                {
                    if (linecount < int(originallines.size())
                        && (originallines[linecount].find("<!-- NEW", 0) != NOT_FOUND
                            || originallines[linecount].find("<!-- FOREACH", 0) != NOT_FOUND))
                    {
                        combinelines.push_back(originallines[linecount]);
                        ++linecount;
                    ...
                    else if (linecount < int(originallines.size())
                             && (conditionOri || !conditionOpen[conditionLvl]))
                    {
                        combinelines.push_back(originallines[linecount]);
                        ++linecount;
                    }
...
void CombineAnimData(string filename,
                     ...
                     bool isHeader)
{
    if (storeline.back().length() != 0 && originallines.back().length() == 0) { storeline.push_back(""); }
    ...
    for (uint k = start; k < storeline.size(); ++k)
    {
        ...
        if ((storeline[k].find("<!-- NEW", 0) == NOT_FOUND
             || storeline[k].find("<!-- FOREACH", 0) == NOT_FOUND)
            && !close)
        {
            if (originallines[linecount].find("<!-- NEW", 0) != NOT_FOUND
                || originallines[linecount].find("<!-- FOREACH", 0) != NOT_FOUND)
        ...
        else if (close && storeline[k].find("<!-- CLOSE -->", 0) != NOT_FOUND)
        {
            ...
            if (isHeader) { newlines.push_back(storeline[k]); }
            else
            {
                ...
                for (uint j = startline; j < combinelines.size() - 1; ++j)
                {
                    ...
                    AD::position position = AnimDataPosition(
                        storeline, characterfile, filename, modcode, filepath, linecount, type);

                    if (position != AD::behaviorfilelist && position != AD::eventnamelist
                        && position != AD::motiondatalist && position != AD::rotationdatalist)
