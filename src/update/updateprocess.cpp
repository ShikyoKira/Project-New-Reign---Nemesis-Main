...
struct arguPack
{
	arguPack(map<string, unique_ptr<map<string, VecStr, alphanum_less>>>& n_newFile, SSSMap& n_parent, MasterAnimData& n_animData, MasterAnimSetData& n_animSetData,
		...
		: newFile(n_newFile), parent(n_parent), animData(n_animData), animSetData(n_animSetData)
#if MULTITHREADED_UPDATE
...
void UpdateFilesStart::RegisterBehavior(shared_ptr<RegisterQueue> curBehavior)
{
	try
	{
			...
			wordFind(curFileName, "_Project") != curFileName.length() - 8)
		{
			...
			if (nemesis::to_lower_copy(curBehavior->file.parent_path().filename().string()).find("characters") == 0)
			{
				...
				for (auto& file : filelist)
				{
					...
					if (!sf::is_directory(curfile))
					{
						...
						if (nemesis::iequals(ext, ".xml") || nemesis::iequals(ext, ".txt"))
						{
							...
							if (nemesis::iequals(ext, "nemesis_animationdatasinglefile") || nemesis::iequals(ext, "nemesis_animationsetdatasinglefile"))
							{
	...
	for (string& modcode : *modlist)
	{
		...
		else
		{
			...
			(*modUpdate)[behavior + node.substr(0, node.find_last_of("."))].FunctionUpdate(modcode, behavior, node, newFile, pack[modcode]->n_stateID[behavior],
				pack[modcode]->parent[behavior], pack[modcode]->statelist[behavior], lastUpdate
#if MULTITHREADED_UPDATE
				, newFileLock, pack[modcode]->stateLock, pack[modcode]->parentLock
#endif
				);
					...
					if ((*stateID[behavior])[ID.first] != ID.second)
					{
						if ((*childrenState[behavior])[parentID][ID.second])
						{
							stateCheck(parent, parentID, behavior, ID.second, stateID[behavior], _stateID,
								(*pack[modcode]->statelist[behavior])[parentID], filename, ID.first, modcode, duplicatedStateList
#if MULTITHREADED_UPDATE
								, duplicatedLock
#endif
								);
		...
		try
		{
			if (isFileExist(directory))
			{
				...
				if (processQueue.size() > 0)
				{
					...
					{
						boost::asio::post(multiThreads, boost::bind(&UpdateFilesStart::SeparateMod, this, directory, each, boost::ref(pack)));
					}
...
void UpdateFilesStart::newAnimProcess(string sourcefolder)
{
	...
	if (CreateFolder(sourcefolder))
	{
		...
		{
			boost::asio::post(multiThreads, boost::bind(&UpdateFilesStart::newAnimUpdate, this, sourcefolder, curCode));
	}
...
#else
			msg = TextBoxMessage(1007) + ": " + to_string(chrono::duration_cast<chrono::seconds>(diff).count()) + " " + TextBoxMessage(1012);
#endif
