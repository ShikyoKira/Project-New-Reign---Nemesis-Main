            ...
            if (!skip)
            {
                if (curline.find("SERIALIZE_IGNORED") == NOT_FOUND)
                {
                    ...
                    if (start)
                    {
                        ...
                        {
                            bool bone = false;
                            std::regex vector4("\\(((?:-|)[0-9]+\\.[0-9]+) ((?:-|)[0-9]+\\.[0-9]+) "
                                                 "((?:-|)[0-9]+\\.[0-9]+) ((?:-|)[0-9]+\\.[0-9]+)\\)");
                            std::smatch match;

                            if (!std::regex_search(curline, match, vector4))
                            {
                                string spaces;
...
bool UpdateFilesStart::AnimDataDisassemble(string path, MasterAnimData& animData)
{
    ...
    {
        string strnum
            = std::regex_replace(string(storeline[0]), std::regex("[^0-9]*([0-9]+).*"), string("\\1"));
                ...
                if (nemesis::iequals(beh, "animationdatasinglefile"))
                {
                    ...
                    for (auto& character : characterlist)
                    {
                        ...
                        else
                        {
                            ...
                            else if (std::regex_match(stemTemp,
                                                        std::regex("^\\$(?!" + curCode + ").+\\$(?:UC|)$")))
                            {
                                ErrorMessage(3023,
                                             "$" + curCode + "$"
...
void UpdateFilesStart::unregisterProcess()
{
    if (!error)
    {
        ...
        else
        {
            ...
            {
                msg = TextBoxMessage(1007) + ": " + to_string(diff.count() / 1000) + " "
                      + TextBoxMessage(1012);
            }
