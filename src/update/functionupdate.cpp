...
)
{
    ...
    string filecheck = std::regex_replace(
                           string(nodefile), std::regex(".+?([0-9]+)\\.[t|T][x|X][t|T]$"), string("\\1"))
                       + ".txt";
    ...
    if (nemesis::iequals("#" + filecheck, nodefile))
    {
        ...
        if (BehaviorFormat.GetFile())
        {
            ...
            while (BehaviorFormat.GetLines(rline))
            {
                ...
                {
                    string stateIDStr = std::regex_replace(
                        string(line),
                        std::regex(".*<hkparam name=\"stateId\">([0-9]+)</hkparam>.*"),
                        string("\\1"));
                ...
                else if (edited)
                {
                    if (IsEventVariable && line.find("numelements=", 0) != NOT_FOUND)
                    {
                        ...
                        {
                            eventcount = stoi(std::regex_replace(
                                string(line), std::regex("[^0-9]*([0-9]+).*"), string("\\1")));
                                 ...
                                 || line.find("<hkparam name=\"attributeDefaults\" numelements=", 0)
                                        != NOT_FOUND)
                        {
                            attributecount = stoi(std::regex_replace(
                                string(line), std::regex("[^0-9]*([0-9]+).*"), string("\\1")));
                        }
                        ...
                        {
                            variablecount = stoi(std::regex_replace(
                                string(line), std::regex("[^0-9]*([0-9]+).*"), string("\\1")));
                        }
                                 ...
                                 || line.find("<hkparam name=\"characterPropertyInfos\" numelements=", 0)
                                        != NOT_FOUND)
                        {
                            characterpropertycount = stoi(std::regex_replace(
                                string(line), std::regex("[^0-9]*([0-9]+).*"), string("\\1")));
                        }
                    }
                    ...
                    bool& openInfo)
{
    ...
    else
    {
        ...
        {
            string tempID = std::regex_replace(
                string(filename), std::regex("[^~]*~" + modcode + "[$]([0-9]+)"), string("\\1"));
