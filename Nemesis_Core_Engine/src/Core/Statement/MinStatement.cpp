...
bool nemesis::MinStatement::TryParse3Components(const nemesis::SemanticManager& manager)
{
...
nemesis::MinStatement::MinStatement(const std::string& expression,
                                    size_t linenum,
    ...
    : nemesis::CompositeStatement(expression, linenum, filepath)
{
    ...
    ThrowSyntaxError("Unsupported Min components");
}

std::string nemesis::MinStatement::GetValue(nemesis::CompileState& state) const
{
