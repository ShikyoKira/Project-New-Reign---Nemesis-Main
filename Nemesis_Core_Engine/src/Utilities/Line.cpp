...
bool nemesis::Line::operator==(const RawChar* str) const noexcept
{
    return base == str;
}
...
bool nemesis::Line::operator==(const nemesis::Line& line) const noexcept
{
...
nemesis::Line nemesis::Line::operator+(const nemesis::Line& line) const noexcept
{
    return s_path ? nemesis::Line(base + line.base, linenum, s_path)
                  : nemesis::Line(base + line.base, linenum, r_path);
}
...
std::string nemesis::Line::GetClassName() const noexcept
{
    ...
    std::string bhv_tmp
        = "behavior_templates" + std::string(1, static_cast<char>(std::filesystem::path::preferred_separator));
    auto pos             = nemesis::isearch(path, bhv_tmp);

    if (pos != NOT_FOUND)
    {
        format = std::string(nemesis::between(
            path, bhv_tmp, std::string(1, static_cast<char>(std::filesystem::path::preferred_separator))));
    }
...
bool nemesis::Wline::operator==(const RawChar* str) const noexcept
{
    return base == str;
}

bool nemesis::Wline::operator==(const RawType& str) const noexcept
{
    return base == str;
}

bool nemesis::Wline::operator==(const nemesis::Wline& line) const noexcept
{
    return base == line.base;
}
...
nemesis::Wline nemesis::Wline::operator+(const nemesis::Wline& line) const noexcept
{
    return s_path ? nemesis::Wline(base + line.base, linenum, s_path)
                  : nemesis::Wline(base + line.base, linenum, r_path);
}
...
std::wstring nemesis::Wline::GetClassName() const noexcept
{
    ...
    std::wstring path    = nemesis::to_lower_copy(nemesis::to_utf8_wstring(GetFilePath()));
    std::wstring bhv_tmp
        = L"behavior_templates" + std::wstring(1, static_cast<wchar_t>(std::filesystem::path::preferred_separator));
    auto pos            = nemesis::isearch(path, bhv_tmp);

    if (pos != NOT_FOUND)
    {
        format = std::wstring(nemesis::between(
            path, bhv_tmp, std::wstring(1, static_cast<wchar_t>(std::filesystem::path::preferred_separator))));
    }
